...
namespace Probe.Controllers
{
    ...
    public class ChoiceQuestionsController : Controller
    {
         ...
         */
        public JsonResult Get([DataSourceRequest]DataSourceRequest request, long aclid, string questionsearch)
        {
            try
            {
                ...
                var questionDTOs = result
                    .GroupBy(q => new { q.Id, q.QuestionTypeId, q.Name, q.Text, q.Tags, q.ACLId, q.OneChoice, q.TestEnabled, q.ChoicesCount })
        ...
        public JsonResult GetQuestionsForAutoComplete(long? gameid)
        {
            try
            {
                //limit the questions to only what the user possesses
                ...
                /*
                 * Depending on whether we are fetching question names for the entire library (gameId passed is null) or 
                 * we are fetching question names for a game, which we must eliminate all the questions that are already in use.
                 */
                if (gameid.HasValue)
                {
                    /*
                     ...
                     */
                    var questionNamesUsed = db.GameQuestion.Where(gq => gq.GameId == (long)gameid)
                                                .Select(gq => new
                                                {
                    ...
                    bool GetTestEnabledQuestionsOnly = true;
                    ...
                    if (db.Game.Find(gameid).GameType.Name == ProbeConstants.MatchGameType)
                    {
                        GetTestEnabledQuestionsOnly = false;
                    }


                    //Get all questions (records that are for QL - UsedInGame = false)
                    //for the game type (match game can use all questions while test, and LMS will only testenabled questions.questions with a correct answer)
                    //for questions that have at least one choice
                    //minus questions used for the game
                    //order by question name
                    var questionNames = db.ChoiceQuestion.Where(cq => cq.AspNetUsersId == loggedInUserId && !cq.UsedInGame
                                                                && (cq.Choices.Any(c => c.Correct) || !GetTestEnabledQuestionsOnly)
                                                                && cq.Choices.Count > 0
                                                                && !questionNamesUsed.Any(qnu => qnu.Name == cq.Name)
                                                                )
                                            .OrderBy(cq => cq.Name)
                                            .Select(cq => cq.Name);
         ...
         */
        public JsonResult GetGameQuestions([DataSourceRequest]DataSourceRequest request, long gameId, long aclid, string questionsearch)
        {
            try
            {
                //limit the questions to only what the user possesses
